...
public class BitVector {
     ...
     * @return mask to get rid of unused bits
     */
    private long getTailMask() {
        int unusedBits = (int) (size % BitsPerElement);
        if (unusedBits == 0) {
            return ~0L;     // keep the whole last word
        } else {
            return (1L << unusedBits) - 1L;
        }
    }
                ...
                if (i + wordShift >= 0 && i + wordShift  < words) {
                    ...
                    if (bitShift != 0) {
                        if (i + wordShift + 1 >= 0 && i + wordShift + 1 < words) {
                            //unsigned right shift for zeros on the left
