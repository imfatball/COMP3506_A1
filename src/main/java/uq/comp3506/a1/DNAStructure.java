...
public class DNAStructure {
    ...
    public DNAStructure(int w) {
        if (w < 0) throw new IllegalArgumentException("w must be >= 0");
        this.w = w;
        this.window = new char[w];
        this.size = 0;
        this.head = 0;
    ...
    public int countRepeats(int k) {
        ...
        final int capPow2 = highestPowerOfTwoAtLeast(Math.max(8, numKmers * 2)); // find the lowest power of 2 that is bigger than number of k-mers
        ...
        for (int i = 0; i < freq.cap; i++) {
            if (freq.used[i] && freq.vals[i] >= 2) repeats++;
        }
        return repeats;
    }
     ...
     * takes in int x and returns smallest 2^n thats bigger than x (min 8)
     */
    ...
    public boolean hasPalindrome(int k) {
        ...
        int forward = 0;
        for (int i = 0; i < k; i++) {
            int idx = (head + i) % w; //logical index
            forward = (forward << 2) | encode(window[idx]); // encode first k bases into binary for comparison
        }
        ...
        for (int i = 0; i < k; i++) {
            int v = encode(window[(head + i) % w]);
            reverseComplement = (reverseComplement >>> 2) | (complementBase2(v) << topShift); // push in complements from the right
        }
        ...
        for (int i = 1; i <= n - k; i++) { // same procedure to roll over the window as countRepeat
            ...
            if (forward == reverseComplement) return true;
        }
